...
급하게 빈칸 포함 scaffold가 필요하면 `allow_todo=true`를 붙인다. Hermes tool `loop_creator_scaffold`는 자동화/테스트 호환성을 위해 직접 scaffold를 생성하지만, slash command UX는 intake gate를 먼저 탄다.
