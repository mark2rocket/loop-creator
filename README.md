...
AI 시대의 병목은 더 많이 생성하는 능력이 아니라, AI가 만든 결과를 믿고 배포할 수 있게 만드는 능력이다.

Loop Creator는 “자율 루프를 알아서 돌리는 플러그인”이 아니다. 더 중요한 역할은 **목표, 검증 기준, 실패 분류, evidence, human gate를 한 패키지로 묶어 좋은 루프가 돌아갈 수밖에 없게 만드는 것**이다.

AI 도입은 보통 계정 도입, 교육, 토큰 사용량 경쟁으로 시작한다. 하지만 곧 산출물 품질, 검증 비용, 업무 파이프라인 미적응 문제가 드러난다. 이때 핵심 비용은 생산량이 아니라 **Verification Tax**다: AI가 만든 결과가 맞는지 확인하고 확신하는 비용이다.

Loop Creator는 그 비용을 줄이기 위해 scaffold 단계부터 Goal Contract, Verification System Design Gate, Learning Trace, eval pack, evidence ledger, failure taxonomy, runner-neutral spec, review report, validator를 강제한다.

여기서 검증은 “AI에게 다시 봐달라고 하는 것”이 아니다. 사람이 먼저 좋은/나쁜 상태의 판단 기준을 세우고, 가능한 부분은 lint/test/schema/type/readback/visual diff처럼 결정적이고 설명 가능한 체크로 바꾸며, LLM/judge review는 아직 결정화하지 못한 잔여 판단에만 쓴다.
...
- AI 산출물의 맥락과 trade-off가 사라져 인지부채와 의도부채가 쌓임

Loop Creator의 사용 효과는 단순 자동화가 아니라 **생산 중심 작업을 검증 레이어 설계와 운영으로 옮기는 것**이다. 전문가는 모든 산출물을 직접 만드는 사람이 아니라, 좋은 것과 나쁜 것을 판별하고 AI가 안전하게 일할 경계를 만드는 사람이 된다.

## AI-native work에서의 효과

| 문제 | Loop Creator가 남기는 것 | 줄어드는 비용 |
|---|---|---|
| Verification Tax | `eval/`, `evidence-ledger.json`, `review-report.md` | 맞는지 확인하고 확신하는 비용 |
| 인지부채 | `goal-contract.md`, `Learning Trace`, `session-handoff.md` | 사람이 이해하지 못한 AI 산출물이 유통되는 위험 |
| 의도부채 | `non_goals`, `risks`, `rejected_alternatives`, approval gate | 왜 그렇게 만들었는지의 맥락 손실 |

AI-native 회사의 조건은 Queryable, Closed loop, Self-improving이다. Loop Creator는 회의, 작은 결정, 시도 결과를 machine-readable한 파일로 남기고, 지난 실패와 맥락이 다음 루프의 입력이 되게 만든다.

- **Queryable:** 목표, evidence, failure, result가 파일로 남는다.
- **Closed loop:** 평가 → 실패 분류 → mutation → 다음 루프로 이어진다.
- **Self-improving:** failure taxonomy와 eval result가 다음 harness의 입력이 된다.
...
- `final/loop-record.json`: Loop Library 스타일의 검색 가능한 루프 record. `slug`, `use_when`, copy-ready `prompt`, `verification`, `terminal_states`, `retirement_rule`, `kill_condition`, `manual_trial`, `next_action_gate`, `related`를 포함한다.
- `final/loop-doctor.md`: 기존 루프를 크게 다시 쓰지 않고 material weakness만 진단/수리하는 Loop Doctor 표면이다.
- `final/loop-audit.md`: read-only portfolio audit 표면. 루프를 `INSUFFICIENT_EVIDENCE`, `KEEP`, `PIVOT`, `RETIRE`, `KILL` 중 하나로 판단하기 위한 scorecard다.
- `final/adaptation-plan.md`: 새 루프를 만들기 전에 기존 published/local loop를 찾고, 가능한 경우 threshold/tool/cadence/check만 바꾸는 adaptation 표면이다.
- `final/hiring-manager.md`: recurring gap, ownership gap, proof gap, manual trial, retirement/kill rule을 보고 이 루프를 “고용”할지 판단한다.
- `state/cross-run-playbook.md`: 한 번 성공한 교훈을 바로 영구 규칙으로 승격하지 않고, 기본 3회 독립 성공/holdout 전까지 candidate lesson으로 보관한다.

이 레이어의 목적은 “좋은 루프가 한 번 돌았다”가 아니라 **새 세션/다른 agent가 같은 run folder만 보고 이어갈 수 있음**과 **만든 루프를 재사용·감사·폐기할 수 있음**을 증명하는 것이다.
...
- Loop Library import: `final/loop-record.json`의 lifecycle status, terminal states, retirement/kill/manual-trial rule, next-action gate
- Doctor/Audit/Adapt/Hiring/Playbook governance surface 존재 여부와 필수 marker
